...
3. **동작 확인**
   ...
   - 2D 스캔 기반일 때는 스캔 포인트가 0.3m 이내로 뭉친 구간을 하나의 장애물 클러스터로 보고 대표점을 cp로 씁니다. 대표점은 장애물 위치에서 추가로 0.35m 외곽으로 띄워 곡선이 장애물 표면에 달라붙지 않게 합니다. 클러스터마다 최근접 대표점을 골라 **반경 약 0.6m의 배제 영역**을 만들고, Bézier 곡선이 이 영역과 겹치면 중간 제어점(P1/P2)만 반복적으로 밀어내어 우회합니다.
