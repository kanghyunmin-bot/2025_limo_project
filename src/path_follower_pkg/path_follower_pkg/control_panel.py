...
class ControlPanel(Node):
    def __init__(self):
        ...
        # Network
        self.current_domain = os.getenv("ROS_DOMAIN_ID", "0")
        self.current_rmw = os.getenv("RMW_IMPLEMENTATION", "rmw_fastrtps_cpp")
        self.current_localhost = os.getenv("ROS_LOCALHOST_ONLY", "0")
        self.host = socket.gethostname()
        self.ip = self._get_ip()

        self.lbl_apply_status = None
        ...
        self.get_logger().info("Control Panel starting...")
    ...
    def on_clicked_point(self, msg):
        ...
        self.points.append((self.point_counter, f"{msg.point.x:.3f}", 
                           f"{msg.point.y:.3f}", time.strftime("%H:%M:%S")))
    ...
    def _gui_main(self):
        try:
            ...
            # STATUS
            status_frm = ttk.LabelFrame(root, text="Current Status", padding=10)
            status_frm.pack(fill="x", padx=10, pady=5)
            ttk.Label(status_frm, text=f"DOMAIN: {self.current_domain}", font=("Arial", 10, "bold")).grid(row=0, column=0, padx=10)
            ttk.Label(status_frm, text=f"RMW: {self.current_rmw}", font=("Arial", 10, "bold")).grid(row=0, column=1, padx=10)
            ttk.Label(status_frm, text=f"LOCALHOST: {self.current_localhost}", font=("Arial", 10, "bold")).grid(row=0, column=2, padx=10)
            ttk.Label(status_frm, text=f"Host: {self.host} | IP: {self.ip}").grid(row=1, column=0, columnspan=3, pady=5)

            # CONFIG
            config_frm = ttk.LabelFrame(root, text="Network Config", padding=10)
            config_frm.pack(fill="x", padx=10, pady=5)
            ttk.Label(config_frm, text="DOMAIN:").grid(row=0, column=0, sticky="e", padx=5)
            domain_var = tk.StringVar(value=self.current_domain)
            ttk.Entry(config_frm, textvariable=domain_var, width=8).grid(row=0, column=1, sticky="w")
            ttk.Label(config_frm, text="RMW:").grid(row=1, column=0, sticky="e", padx=5)
            rmw_var = tk.StringVar(value=self.current_rmw)
            ttk.Combobox(config_frm, textvariable=rmw_var, width=20,
                         values=["rmw_fastrtps_cpp", "rmw_cyclonedds_cpp"]).grid(row=1, column=1, sticky="w")
            ttk.Label(config_frm, text="LOCALHOST:").grid(row=2, column=0, sticky="e", padx=5)
            localhost_var = tk.StringVar(value=self.current_localhost)
            ttk.Combobox(config_frm, textvariable=localhost_var, width=8, values=["0", "1"]).grid(row=2, column=1, sticky="w")
            
            btn_frame = ttk.Frame(config_frm)
            btn_frame.grid(row=0, column=2, rowspan=3, padx=20)
            ttk.Button(btn_frame, text="Apply", command=lambda: self._apply_bashrc(domain_var.get(), rmw_var.get(), localhost_var.get())).pack(pady=5)
            ttk.Button(btn_frame, text="Restart", command=self._restart_nodes).pack(pady=5)
            
            self.lbl_apply_status = ttk.Label(config_frm, text="")
            self.lbl_apply_status.grid(row=3, column=0, columnspan=3, sticky="w", pady=5)
    ...
    def _apply_bashrc(self, domain, rmw, localhost):
        try:
            ...
            with open(bashrc, 'w') as f:
                f.writelines(lines)
            if self.lbl_apply_status:
                self.lbl_apply_status.config(text=f"Saved: DOMAIN={domain}, RMW={rmw}", foreground="green")
            self.get_logger().info(f"Applied: DOMAIN={domain}")
        except Exception as e:
            if self.lbl_apply_status:
                self.lbl_apply_status.config(text=f"Error: {e}", foreground="red")

    def _restart_nodes(self):
        if not messagebox.askyesno("Restart", "Restart all nodes?"):
            return
        try:
            subprocess.run(['pkill', '-f', 'path_follower'])
            time.sleep(0.5)
            cmd = 'gnome-terminal -- bash -c "source ~/.bashrc && source ~/path_follower/install/setup.bash && ros2 launch path_follower_pkg path_follower.launch.py; exec bash"'
            subprocess.Popen(cmd, shell=True)
            messagebox.showinfo("Done", "New terminal opened!\n\nCurrent terminal:\nsource ~/.bashrc")
